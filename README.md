...
*   **Trên Windows:**
    Tải xuống và cài đặt `sox` từ trang web chính thức: [https://sox.sourceforge.net/](https://sox.sourceforge.net/)
    Đảm bảo rằng `sox.exe` nằm trong thư mục gốc của dự án hoặc trong biến môi trường PATH của bạn. (Trong dự án này, `sox.exe` được đặt sẵn trong thư mục app để tiện lợi).
