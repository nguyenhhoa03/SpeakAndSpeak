...
- `discrimination.py` - Module xử lý phân biệt âm thanh
- `arpabet_ipa_database.csv` - Cơ sở dữ liệu chuyển đổi ARPABET sang IPA
- `ipa_confusion_groups.yaml` - Nhóm các âm dễ nhầm lẫn trong IPA

### Cải tiến Windows:
- Sử dụng `/Y` flag cho lệnh copy để tự động ghi đè file nếu đã tồn tại
- Tự động copy `SpeakAndSpeak.iss` vào dist, giảm bước thủ công khi tạo trình cài đặt

### Cải tiến Linux:
- Sử dụng `-f` flag cho lệnh cp để ghi đè file tự động
- Thêm các file dữ liệu mới vào quá trình build
